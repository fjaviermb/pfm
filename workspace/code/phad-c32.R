...
logger <- function(message = "None", time=TRUE) {
  if( time ) {
...
labelTestingDS <- function(testing.raw.ds, label.attacklist.raw.ds, cache = FALSE, root.dir = getwd()) {
  ...
  if( is.null(cache)) {
    ...
    for(index in 1:nrow(label.attacklist.raw.ds)){
      ...
      logger( paste("[",index, "/",nrow(label.attacklist.raw.ds),"] Processing attack..." ))
  ...
  for(indexObs in 1:scoreNRows) {
    ...
    if(! is.na(valueFeature ) ) { 
      ...
      if( model.ds[[indexFeatureOnModel]]$contains(valueFeature) ) {
        ...
        # str(lastAnomaly[feature] -> lastAnomaly
        # timestamp -> initialTimestamp
        timestamp<-testing.raw.ds.timestamp[[1]][indexObs]
        logger(paste("ANALISIS","|",indexFeatureOnModel,"|",timestamp,"|",lastAnomaly,"|","NA","|","NA","|","NA","|","NA","|",valueFeature,"|","INCLUDED",sep=""),FALSE)
        ...
        logger(paste("ANALISIS","|",indexFeatureOnModel,"|",timestamp,"|",lastAnomaly,"|",t,"|","NA","|","|",scores[indexObs],"|",valueFeature,"|","CALCULATED", sep=""),FALSE)
    
      }
      
    } else {
      logger(paste("ANALISIS","|",indexFeatureOnModel,"|",initialTimestamp,"|",lastAnomaly,"|","NA","|","NA","|","NA","|","NA",valueFeature,"|","INVALID",sep=""),FALSE)
      numNA <- numNA +1
