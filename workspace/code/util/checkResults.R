...
getFPFNTable <- function(testing.raw.scored.ds, th = 0) {
  totalPackets <- dim(testing.raw.scored.ds.log)[[1]]
  
  testing.raw.scored.ds.reduced <- testing.raw.scored.ds %>% select(attack, scoringNorm)
  
  thValue <- th
  totalPackages <- dim(testing.raw.scored.ds)[[1]]
  foPackages <- dim(testing.raw.scored.ds.reduced %>% filter(attack == FALSE & scoringNorm <= thValue))[[1]]
  toPackages <- dim(testing.raw.scored.ds.reduced %>% filter(attack == TRUE  & scoringNorm >  thValue))[[1]]
  fnPackages <- dim(testing.raw.scored.ds.reduced %>% filter(attack == FALSE & scoringNorm >  thValue))[[1]]
  fpPackages <- dim(testing.raw.scored.ds.reduced %>% filter(attack == TRUE  & scoringNorm <= thValue))[[1]]
...
#4.5.2.1.2
renderAttackTypeDetection <- function(scored.ds, threshold = 0, render = FALSE ) {
...
#4.5.2.2 - Grpah
renderAttackTypeDetectionDetailed <- function(scored.ds, attackType, threshold = 0, render = FALSE ) {
...
#4.5.2.2 - Table
renderAttackTypeDetectionDetailedTable <- function(scored.ds, attackType, threshold = 0, render = FALSE ) {
...
#4.5.2.1.1
renderCoverageType <- function(scored.ds, threshold = 0, render = FALSE ) {
...
renderFPFNTable <- function(scored.ds, threshold = 0, render = TRUE ) {
...
renderFPFNTableComparison  <- function(scored.ds, threshold.1 = 0, threshold.2 = 0 ) {
...
renderFPFNByAttackTable <- function(scored.ds, threshold = 0, render = TRUE ) {
...
renderFPFNByAttackTableComparison <- function(scored.ds, threshold.1 = 0, threshold.2 = 0, filename ) {
